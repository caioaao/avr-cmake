...
** Pre-processing
   ...
   #+begin_src c++ :tangle examples/buzzer/src/main.cpp :mkdirp t
     #include <Arduino.h>
   ...
   #+begin_src c++ :tangle examples/balancing/src/main.cpp :mkdirp t
     #include <Arduino.h>
...
*** Compiling main
    ...
    #+begin_src cmake :tangle examples/progmemTest/src/CMakeLists.txt
      <<cmake-conf-cross-compilation-variables>>
      cmake_minimum_required(VERSION 3.0.0)
    ...
    #+begin_src sh :dir examples/buzzer :results output verbatim
      out_dir=`mktemp -d`
